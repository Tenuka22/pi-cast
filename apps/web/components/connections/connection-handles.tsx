...
export function ConnectionHandles({
  ...
  const defaultHandles: Array<{ id: string; type: ConnectionHandleType; label?: string }> = handles || (() => {
    ...
    if (['chart', 'control', 'limit', 'variable', 'logic', 'comparator', 'constraint', 'shape', 'table', 'piecewise-limiter', 'piecewise-builder'].includes(blockType)) {
      result.push({ id: `${blockId}-input`, type: 'input' });
    }

    // All blocks that can send connections get output handles
    if (['equation', 'limit', 'variable', 'logic', 'comparator', 'constraint', 'control', 'piecewise-limiter', 'piecewise-builder'].includes(blockType)) {
  ...
  const isValidTarget = isConnecting && connectingFromType && (
    (connectingFromType === 'equation' && ['chart', 'control', 'equation', 'limit', 'variable', 'constraint', 'table', 'piecewise-limiter'].includes(blockType)) ||
    (connectingFromType === 'variable' && ['chart', 'equation'].includes(blockType)) ||
    (connectingFromType === 'control' && ['shape', 'limit', 'comparator', 'equation'].includes(blockType)) ||
    (connectingFromType === 'limit' && ['chart', 'table', 'logic'].includes(blockType)) ||
    (connectingFromType === 'logic' && ['logic', 'chart', 'shape', 'comparator', 'equation'].includes(blockType)) ||
    (connectingFromType === 'comparator' && ['logic', 'chart', 'shape', 'comparator', 'equation', 'table'].includes(blockType)) ||
    (connectingFromType === 'constraint' && ['equation', 'chart', 'logic', 'comparator', 'constraint', 'table'].includes(blockType)) ||
    (connectingFromType === 'piecewise-limiter' && ['piecewise-builder'].includes(blockType)) ||
    (connectingFromType === 'piecewise-builder' && ['chart', 'table', 'logic', 'shape'].includes(blockType))
  );
