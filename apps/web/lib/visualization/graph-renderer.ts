 ...
 */
export function evaluateFunction(equation: string, variables: Record<string, number>): number {
  try {
...
export function graphToPixels(
  ...
  const pixelY = config.height - ((point.y - config.yAxis.min) / yRange) * config.height;
...
export function renderGraph(
  ...
  equations: Array<{ equation: string; variables: Record<string, number>; options?: PlotOptions }>,
 ...
 */
export function getDefaultConfigForEquation(equationType: string): Partial<GraphConfig> {
  switch (equationType) {
    case 'trigonometric':
      return {
        xAxis: { min: -2 * Math.PI, max: 2 * Math.PI, label: 'x', showLabels: true, showGrid: true },
        yAxis: { min: -2, max: 2, label: 'y', showLabels: true, showGrid: true },
      };
    case 'exponential':
      return {
        xAxis: { min: -5, max: 5, label: 'x', showLabels: true, showGrid: true },
        yAxis: { min: -10, max: 100, label: 'y', showLabels: true, showGrid: true },
      };
    default:
      return {};
  }
